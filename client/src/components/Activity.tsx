...
export default function Activity(props: ActivityProps) {
  ...
  const maxHeight = 9; // Maximum height of a bar
  const barHeights = {
  ...
  return (
    <>
      <div className="flex flex-col ml-1 font-bold min-w-[416px]">
        <h3 className="ml-3 text-2xl">Activity</h3>
        <div className="flex border border-[#004E89] rounded-xl mt-2 h-[203px] px-8">
          {Object.entries(barHeights).map(([day, height]) => (
            <div
              key={day}
              className="px-3 flex flex-col mx-auto justify-end p-2"
            >
              <div
                style={{ height: `${height}rem` }}
                className={`${barColor(day)} w-8 rounded-xl`}
              ></div>
              <p className="mx-auto mt-2">{day}</p>
            </div>
          ))}
        </div>
      </div>
